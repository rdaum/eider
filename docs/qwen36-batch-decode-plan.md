...
- routed-MoE route, indexed or grouped W4A4 gate/up, SM12x down, and
  shared-expert storage; and
...
5. batched router top-k, indexed or grouped W4A4 routed gate/up, SM12x routed
   down, shared expert, and fused FFN finalization;
...
The current workspace constructor uses indexed W4A4 routed gate/up for one-row
decode and grouped W4A4 for larger batches, with SM12x routed down and the
NVFP4 shared-expert plan. Add a true batched path for FP8 experts before moving
mixed-storage checkpoints onto it; do not implement that support as a host loop
over sequences.
