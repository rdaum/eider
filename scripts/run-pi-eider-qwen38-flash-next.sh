...
model="${EIDER_SERVED_MODEL:-eider-qwen3.8-flash-next}"
thinking="${PI_EIDER_THINKING:-medium}"
pi_tools="${PI_EIDER_TOOLS:-read,bash,edit,write,grep,find,ls}"
...
if command -v curl >/dev/null 2>&1 && ! curl --fail --silent --show-error "$health_url" >/dev/null; then
  ...
  echo "start it first with scripts/run-eider-qwen38-flash-next.sh" >&2
  exit 1
fi
