...
pub struct QwenXmlGrammarFactory {
...
impl QwenXmlGrammarFactory {
    pub fn new(tokenizer: &Tokenizer, vocab: usize) -> Result<Self> {
    ...
    pub fn build(&self, tools: &[ChatTool]) -> Result<Option<QwenXmlToolGrammar>> {
        if tools.is_empty() {
            return Ok(None);
        }
        let (source, triggers) =
...
impl QwenXmlToolGrammar {
    ...
    pub fn mask(&mut self) -> Result<Option<Vec<u32>>> {
    ...
    pub fn commit(&mut self, token: u32) -> Result<()> {
        if !self.active {
    ...
    pub fn token_allowed(mask: &[u32], token: u32) -> bool {
...
mod tests {
    use super::*;
    use crate::chat::ChatFunctionDefinition;
    use serde_json::json;
    use std::path::Path;
