//! Shared paged sequence storage for Bonsai.

use super::{BonsaiDecodeState, BonsaiModel};
...
impl BonsaiSequence {
    pub fn admit(
        ...
        let AdmissionOutcome::Admitted(cache_id) = outcome else {
            return Err(Error::Format {
                label: "Bonsai sequence admission",
    ...
    pub fn finish(self, cache: &mut BonsaiSequenceCache) -> Result<()> {
        let Self {
            cache_id,
            mut page_table,
            state,
...
pub fn new_bonsai_sequence_cache(
    model: &BonsaiModel,
    sequence_capacity: usize,
    max_context_tokens: usize,
) -> Result<BonsaiSequenceCache> {
    let config = model.config();
    if sequence_capacity == 0 || max_context_tokens == 0 {
        return Err(Error::Shape {
            label: "Bonsai sequence cache",
    ...
    let page_slots = sequence_capacity
        ...
        .ok_or_else(|| Error::Shape {
            label: "Bonsai sequence cache pages",
    ...
    let fixed = model
        ...
        .ok_or_else(|| Error::Shape {
            label: "Bonsai sequence cache bytes",
...
pub(crate) fn bonsai_cache_error(error: CacheError<Error>) -> Error {
    Error::Format {
        label: "Bonsai sequence cache",
