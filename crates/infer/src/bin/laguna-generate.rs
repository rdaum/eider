...
use eider_inference::laguna::{LagunaSequence, new_laguna_sequence_cache};
...
fn main() -> Result<()> {
    ...
    let program = args
        ...
        .unwrap_or_else(|| "laguna-generate".to_string());
    let model_dir = args
        .next()
        .map(PathBuf::from)
    ...
    let model = LagunaModel::load(&model_dir)?;
    println!(
