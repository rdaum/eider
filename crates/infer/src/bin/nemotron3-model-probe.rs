...
fn parse_bf16_storage(index: usize) -> eider_cuda::Result<Nemotron3Bf16Storage> {
    match std::env::args().nth(index).as_deref().unwrap_or("bf16") {
        ...
        value => Err(eider_cuda::Error::Format {
...
fn parse_fp8_storage(index: usize) -> eider_cuda::Result<Nemotron3Fp8Storage> {
...
fn parse_count(index: usize, label: &'static str, default: usize) -> eider_cuda::Result<usize> {
    std::env::args()
        .nth(index)
        .map(|value| {
            value
