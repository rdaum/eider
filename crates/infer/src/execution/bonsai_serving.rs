...
struct ActiveRequest<'tokenizer> {
    ...
    sequence_id: Option<BonsaiSequenceId>,
    sampler: Sampler,
...
impl<'model, 'template> BonsaiChatService<'model, 'template> {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            sequences: BonsaiSequencePool::new(),
            sequence_cache,
            prefill_workspace,
        })
    }
    ...
    pub fn cancel_request(&mut self, id: BonsaiRequestId) -> BonsaiCancelOutcome {
        ...
        let released = request.sequence_id.and_then(|sequence_id| self.sequences.release(sequence_id).ok()).map_or(0, |sequence| { let bytes = sequence.device_bytes(); if let Err(error) = sequence.finish(&mut self.sequence_cache) { warn!(%error, request_id = id.get(), "failed to release cancelled Bonsai sequence"); } bytes });
...
/// lifecycle records. Bonsai sequence identities remain internal to inference.
pub struct BonsaiEngineService<'model, 'template> {
    inner: BonsaiChatService<'model, 'template>,
    ids: BTreeMap<u64, BonsaiRequestId>,
}

impl<'model, 'template> BonsaiEngineService<'model, 'template> {
    /// Wraps a Bonsai chat service for consumption by an engine actor.
    pub fn new(inner: BonsaiChatService<'model, 'template>) -> Self {
        Self {
            inner,
            ids: BTreeMap::new(),
        }
...
impl EngineService for BonsaiEngineService<'_, '_> {
